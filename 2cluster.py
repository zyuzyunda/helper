...
# Далее так как навыков более 5к, для понятной интерпретации необходимо было кластеризировать
# Так как тяжело было определить кол-во кластеров(надо как-то адекватно объяснить почему только 2 метода выбрала),
# были выбраны протетисрованы HDBSCAN и Агломеративная кластеризация.
# По итогу лучший вариант был HDBSCAN, так как Агломеративная кластеризация выделяла слишком много и видимо члишком чувствителен к мусору.
# хотя мусор был почищен, но очень много разных формулировок, хз почему надо обяхснить номрально.
# здесь вставлю картинку визуализации кластеризации агломеративная и HDBSCAN
# Потом я начала работать только HDBSCAN и дополнительно применила UMAP.
# Дальше так как выделялись больше кластеры где несколько тысяч навыков, было принято решение провести двойную кластризацию 
# Надо вставить примеры.
